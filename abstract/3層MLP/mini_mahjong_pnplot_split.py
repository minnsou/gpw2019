...
def value_iteration(n, m, l, gamma): # 価値反復法により状態の価値を求める関数
    ...
    n_hand = len(hand_nml)
    value_state = [1 if is_win_nml[i] else 0 for i in range(len(state_nml))] # 状態の価値はあがっていればvalueは1、いなければ0
...
def one_hot_vector3(hists, n, l=4): # one_hot_vector2に近いけど、持ってる枚数より小さい数も1で埋めた(手牌１つがn * lの行列に対応)
