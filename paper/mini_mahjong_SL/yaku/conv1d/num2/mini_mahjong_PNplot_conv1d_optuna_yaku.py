...
from keras.layers import Input, Dense, Flatten, Dropout, Conv1D
...
def train_policy_network(num, input_shape, n_study, EPOCHS):
    ...
    def create_model(trial):
        ...
        # We define our myconv1d
        model = Sequential()
        model.add(layers.InputLayer(input_shape=input_shape))
        n_conv1d_layers = trial.suggest_int('n_conv1d_layers', 1, 4)
        for i in range(n_conv1d_layers):
            num_filters = trial.suggest_int('n_filters_|{}'.format(i), 8, 64)
            model.add(Conv1D(filters=num_filters, kernel_size=3, strides=1, padding='same', activation='relu'))
        model.add(Flatten())
